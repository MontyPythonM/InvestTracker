...
internal sealed class AddEdoBondAssetHandler : ICommandHandler<AddEdoAsset>
{
    ...
    public async Task HandleAsync(AddEdoAsset command, CancellationToken token)
    {
        ...
        var edo = portfolio.AddEdoTreasuryBond(Guid.NewGuid(), command.Volume, command.PurchaseDate, 
        ...
        await _messageBroker.PublishAsync(new Events.FinancialAssetAdded(edo.Id, edo.AssetName, portfolio.Id, 
