...
internal sealed class GetEdoAmountChartHandler : IQueryHandler<GetEdoAmountChart, EdoAmountChart>
{
    ...
    public async Task<EdoAmountChart> HandleAsync(GetEdoAmountChart query, CancellationToken token = default)
    {
        ...
        var inflationRates = await _inflationRateRepository
            .GetChronologicalInflationRatesAsync(new DateRange(edo.PurchaseDate, edo.GetRedemptionDate()), token);
        ...
        return new EdoAmountChart(amounts, edo.Symbol, edo.Currency);
    }
}